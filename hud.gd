...
const INITIAL_ROF_VALUE := 1.0
const ROF_COST_INCREMENT := 5
const ROF_VALUE_INCREMENT := 10.0 / 60.0
const INITIAL_TRACK_COST := 5
const INITIAL_TRACK_VALUE := 45.0
const TRACK_COST_INCREMENT := 5
const TRACK_VALUE_INCREMENT := 5.0

# Shield constants
const SHIELD_REGEN_PULSE_THRESHOLD := 1.0

# Typed container for per-turret upgrade state
class TurretUpgradeData:
	var rof_cost: int
	var track_cost: int

	func _init(initial_rof: int, initial_track: int) -> void:
		rof_cost = initial_rof
		track_cost = initial_track
...
var shield_regen_rate: float = 0.5
